...
path = "C:/Users/Majed Tamim/Desktop/Image Reader/Images/Data/Age_Gender/UTKFace/UTKFace"
images = []
age = []
gender = []
...
history_age = age_model.fit(x_train_age, y_train_age,
                            validation_data=(x_test_age, y_test_age), epochs=600)

age_model.save('age_model_600epochs.h5')
...
history_gender = gender_model.fit(x_train_gender, y_train_gender,
                                  validation_data=(x_test_gender, y_test_gender), epochs=600)

gender_model.save('gender_model_600epochs.h5')
...
# Test the model gender
my_model = load_model('gender_model_600epochs.h5', compile=False)
...
# Test the model age
my_modelA = load_model('age_model_600epochs.h5', compile=False)

predictionsA = my_modelA.predict(x_test_age)
y_predA = (predictionsA >= 0.5).astype(int)[:, 0]

from sklearn import metrics

print("Accuracy = ", metrics.accuracy_score(y_test_age, y_predA))
...
cm = confusion_matrix(y_test_age, y_predA)
sns.heatmap(cm, annot=True)